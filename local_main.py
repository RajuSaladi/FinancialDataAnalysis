...
if __name__ == "__main__":
    ...
    if not type_interval in ['minute','second','hour','day']:
        type_interval = "minute"
        value_interval = 5
    ...
    while(i < 20):
        ...
        #delta = datetime.timedelta(days = value_interval*(SECONDWINDOW+DELTA_HYST_TIME+2))
        #delta = dt.timedelta(eval(type_interval+'s') = value_interval*(21+5))
        #from_date = now - delta
        #from_date_str = from_date.strftime('%Y-%m-%d %H:%M:%S')
        #to_date_str = now.strftime('%Y-%m-%d %H:%M:%S')

        recorded_data = df.iloc[i:i + (SECONDWINDOW+DELTA_HYST_TIME+2)]
        #pdb.set_trace()
